...
async function set_bridge_address( asset_pool_instance, bridge_logic_address, account){
  ...
  console.log(asset_pool_instance)
  //create signature
  let encoded_message = get_message_to_sign(
      ["address"],
  ...
  //NOTE Sig tests are in MultisigControl
  await asset_pool_instance.methods.set_bridge_address(bridge_logic_address, nonce, sig_string).send({from:wallet_address,
    gasPrice:"150000000000", gas: "2000000"});
}
async function configure_signers() {
    ...
    let asset_pool_instance = new web3_instance.eth.Contract(pool_abi, bridge_address_file.asset_pool);
    await set_bridge_address(asset_pool_instance, bridge_address_file.logic_1, "0xE8a3Ee358B578a0f1CDB2Fd10D56fBb133926360");
