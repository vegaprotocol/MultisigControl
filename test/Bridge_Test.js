...
contract("Vega_Bridge_ERC20",  (accounts) => {
    it("should exist", async () => {
        ...
        console.log("attempting to deposit token before whitelist");
        try {
            await vega_bridge_erc20_instance.deposit_asset(VUSD_TEST.address, 0, token_balance, "0x00");
        }catch (e) {
            console.log("deposit before whitelist failed");
        }
        ...
        let deposit_receipt = await vega_bridge_erc20_instance.deposit_asset(VUSD_TEST.address, 0, token_balance, "0x00");
